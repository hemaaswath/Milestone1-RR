"""
Phase 8 Streamlit App - No Pandas Version

This version works without pandas to avoid Windows installation issues.
Charts and analytics will be simplified but the core functionality works.
"""
...
def display_header():
    ...
    st.markdown('<div class="sub-header">Phase 8: Streamlit Demo App - No Pandas Version</div>', unsafe_allow_html=True)
...
def display_simple_charts(recommendations_data):
    """Display simple charts without pandas or plotly."""
    recommendations = recommendations_data.get("data", {}).get("recommendations", [])
    ...
    st.header("📊 Simple Analytics")
    
    # Create simple bar charts using streamlit
    col1, col2 = st.columns(2)
    ...
    with col1:
        ...
        for restaurant in recommendations:
            ...
            st.progress(rating / 5.0, text=f"{name}: {rating}/5")
    
    with col2:
        st.subheader("💰 Cost Analysis")
        for restaurant in recommendations:
            cost = restaurant.get('cost_for_two', 0)
            name = restaurant.get('restaurant_name', 'Unknown')
            max_cost = 2000
            st.progress(min(cost / max_cost, 1.0), text=f"{name}: ₹{cost:,}")
    
    # Display summary statistics
    st.subheader("📊 Summary Statistics")
    
    # Calculate statistics manually
    ratings = [r.get('rating', 0) for r in recommendations]
    costs = [r.get('cost_for_two', 0) for r in recommendations]
...
def main():
    ...
    with st.sidebar:
        ...
        **Features:**
        ...
        - ✅ Beautiful UI (simplified charts)
        ...
        if st.session_state.recommendations:
            ...
            if recs:
                ratings = [r.get('rating', 0) for r in recs]
                costs = [r.get('cost_for_two', 0) for r in recs]
                st.metric("Last Search", f"{len(recs)} results")
                st.metric("Avg Rating", f"{sum(ratings)/len(ratings):.1f}" if ratings else "0.0")
                st.metric("Avg Cost", f"₹{sum(costs)/len(costs):.0f}" if costs else "₹0")
        else:
    ...
    with tab2:
        if st.session_state.recommendations:
            display_simple_charts(st.session_state.recommendations)
        else:
    ...
    <div style="text-align: center; color: #6b7280; font-size: 0.875rem;">
        Phase 8 Streamlit Demo App • Restaurant Recommendation System • No Pandas Version
    </div>
    """, unsafe_allow_html=True)
